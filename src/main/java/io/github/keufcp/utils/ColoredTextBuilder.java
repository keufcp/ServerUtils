  ...
  public static class MobCapColors {
    ...
    public static Text createMobCapDisplay(long current, int cap, int spawnChunkCount) {
      return Text.empty()
          .append(Text.literal("  "))
          .append(
              Text.literal(String.valueOf(current)).formatted(getCurrentCountColor(current, cap)))
          .append(Text.literal("/").formatted(StatusColors.getNormalColor()))
          .append(
              Text.literal(String.valueOf(cap)).formatted(getCapLimitColor(cap, spawnChunkCount)));
    }
  }
