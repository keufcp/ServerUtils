 ...
 * <p>
 * /suMobCap コマンドの登録・実行処理を担当し、
 * サーバー内の各ディメンションのモンスターキャップ情報を表示する。
 * </p>
 */
public class ServerUtilsMobCapCommand {
    ...
    private static final SuggestionProvider<ServerCommandSource> DIMENSION_SUGGESTIONS =
        (context, builder) -> CommandSource.suggestMatching(
            DimensionResolver.getSuggestions(context.getSource()),
            builder
        );
    ...
    private static void registerCommand(CommandDispatcher<ServerCommandSource> dispatcher,
        ...
        dispatcher.register(CommandManager.literal("suMobCap")
            .requires(source -> source.hasPermissionLevel(ServerUtilsMidnightConfig.mobcapPermissionLevel))
            .executes(context -> executeAllDimensionsCommand(context.getSource(), false))
            .then(CommandManager.literal("debug")
                .executes(context -> executeAllDimensionsCommand(context.getSource(), true))
            )
            .then(CommandManager.argument("dimension_alias_or_id", StringArgumentType.string())
                .suggests(DIMENSION_SUGGESTIONS)
                .executes(context -> executeSpecificDimensionCommand(
                    context.getSource(),
                    StringArgumentType.getString(context, "dimension_alias_or_id"),
                    false))
                .then(CommandManager.literal("debug")
                    .executes(context -> executeSpecificDimensionCommand(
                        context.getSource(),
     ...
     * 単一ディメンションコマンドの実行.
     */
    private static int executeSpecificDimensionCommand(ServerCommandSource source,
                                                     String dimensionString,
                                                     boolean debug) throws CommandSyntaxException {
        ...
        Text output;
        if (ColoredTextBuilder.shouldUseColoredText(source)) {
            output = MobCapFormatter.createColoredSingleDimensionOutput(world, dimensionDisplayName, debug);
        } else {
            output = Text.literal(MobCapFormatter.createPlainSingleDimensionOutput(world, dimensionDisplayName, debug));
        }
    ...
    private static SimpleCommandExceptionType createInvalidDimensionException() {
        return new SimpleCommandExceptionType(Text.literal(ServerUtils.LANG.get("mobcap.error.invalid_dimension")));
    }
}
